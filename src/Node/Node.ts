...
export class Node extends EventEmitter
{
    ...
    constructor(public readonly id: string, private readonly template: NodeTemplate)
    {
        ...
        this.container.style.backgroundImage = 'linear-gradient(' + color + ', ' + this.adjustColor(color, -50) + ' 35%)';

        // Apply header text.
        this.headerTitle.innerHTML    = template.name;
        this.headerSubTitle.innerHTML = this.getTypeName(template.type);
     ...
     * @private
     */
    private getTypeName(type: NodeType): string
    {
        switch (type) {
            case NodeType.ACTION: return 'Action';
            case NodeType.CONDITION: return 'Branch';
            case NodeType.SUCCESS: return 'Successful finalization';
            case NodeType.FAILURE: return 'Failed finalization';
        }
    }
